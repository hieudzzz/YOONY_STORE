...
import { ConfigProvider, Pagination, Switch, Upload } from "antd";
...
const BannerList: React.FC = () => {
  ...
  const fetchBanners = async () => {
    try {
      ...
      const { data } = await instance.get(`banners?page=${page}`);
  ...
  const onSubmit = async (dataForm: IBanner) => {
    try {
      ...
      if (fileList[0]?.url) {
        ...
        if (!check_type_image) {
          toast.error("Chỉ hỗ trợ định dạng .jpg, .jpeg, .png , .svg và .webp");
          return;
        }
      ...
      <div className="overflow-x-auto rounded-lg">
        <Table>
          <Table.Head className="text-center">
            <Table.HeadCell className="bg-[#F4F7FA] text-secondary/75 text-sm font-medium text-nowrap">
              STT
            </Table.HeadCell>
            <Table.HeadCell className="bg-[#F4F7FA] text-secondary/75 text-sm font-medium text-nowrap">
              Hình Ảnh
                ...
                return (
                  ...
                  >
                    ...
                    <Table.Cell>
                      <img
                        src={banner.image}
                        className="mx-auto w-52 object-cover rounded-sm"
                      />
                    </Table.Cell>
                    <Table.Cell>
                      <ConfigProvider
