...
type Props = {
  activeTab: string;
  setPreparingCount:(count:number)=>void
};
const OrderListPreparing = ({ activeTab,setPreparingCount }: Props) => {
  const [ordersPreparing, setOrdersPreparing] = useState<Orders[]>([]);
  const [searchParams, setSearchParams] = useSearchParams();
  const page = parseInt(searchParams.get("page") || "1");
  const [meta, setMeta] = useState<IMeta>();
  const [checkedList, setCheckedList] = useState<number[]>([]);
  const checkAll = ordersPreparing.length === checkedList.length;
  const indeterminate =
    checkedList.length > 0 && checkedList.length < ordersPreparing.length;
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        const {
          data: { data: response },
        } = await instance.get("admin/orders?status=preparing_goods");
        setOrdersPreparing(response.data);
        setMeta(response);
        setPreparingCount(response.data.length);
      } catch (error) {
        console.log(error);
      }
    })();
  ...
  const handleUpdateStatusOrderNext = async (status: string) => {
    try {
      ...
      setOrdersPreparing(ordersPreparing.filter(
        (item: Orders) => !checkedList.includes(item.id!)
      ))
      setCheckedList([])
      toast.success('Cập nhật lên trạng thái: Đang vận chuyển')
    } catch (error) {
      console.log(error);
    }
  };
          ...
          <Table className="border-b border-[#E4E7EB]">
            ...
            <Table.Body className="divide-y">
              {ordersPreparing.length === 0 ? (
                ...
                ordersPreparing.map((order, index) => {
                  return (
                    <Table.Row
        ...
        >
          {checkedList.length > 0 && (
            <button
              ...
              onClick={() => handleUpdateStatusOrderNext("preparing_goods")}
            >
              Đang vận chuyển
            </button>
          )}
