...
type Props = {
  activeTab: string;
  setConfirmedCount:(count:number)=>void
};
const OrderListConfirmed = ({ activeTab,setConfirmedCount }: Props) => {
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        setMeta(response);
        setConfirmedCount(response.data.length);
      } catch (error) {
        console.log(error);
      }
    })();
