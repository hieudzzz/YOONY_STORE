...
const OrderListPending = ({ activeTab,setPendingCount }: Props) => {
  ...
  const checkAll = ordersPending.length === checkedList.length;
  const indeterminate =
    checkedList.length > 0 && checkedList.length < ordersPending.length;
  ...
  const handleUpdateStatusOrderNext = async (status: string) => {
    try {
      ...
      setOrdersPending(
        ordersPending.filter((item: Orders) => !checkedList.includes(item.id!))
      );
      setCheckedList([]);
      toast.success("Đã xác nhận các đơn hàng đã chọn!");
    } catch (error) {
      console.log(error);
    }
  };
        ...
        >
          {checkedList.length > 0 && (
            <button
              ...
              onClick={() => handleUpdateStatusOrderNext("pending")}
            >
              Xác nhận đơn hàng
            </button>
          )}
