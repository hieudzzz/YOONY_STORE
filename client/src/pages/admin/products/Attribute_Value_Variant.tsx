        ...
        if (av.attribute_id) {
          ...
          if (attributeValuesCache[av.attribute_id]) {
            ...
            setAttributeValuesCache((prev) => ({
              ...prev,
              [av.attribute_id]: values,
            }));
          }
          initialAttributeValuesMap[i] = values;
          setValue(
            `variants.${index}.attribute_values.${i}.attribute_value_id`,
            av.attribute_value_id
          );

          const attribute = attributes.find(
            (attr) => attr.id === av.attribute_id
          );
    ...
    <>
      {fields.map((field, attrIndex) => {
        ...
        const optionsAttribute = availableAttributes.map((attribute) => ({
          ...
          type: attribute.type === "color" ? "color" : "button",
        }));
            ...
            <div className="grid grid-cols-3 gap-[15px]" key={field.id}>
              ...
              <div>
                ...
                <Select
                  ...
                  options={optionsAttributeValue.map((option) => ({
                    ...option,
                    type: "button",
                  }))}
                />
